...
describe('Service: reviewService', function () {
  // Simula el token de autenticación
  Cookies.set('token', '"eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJfaWQiOiI1NWMwNjcyMjU5MTkwM2UzMDU0Mzg0OGEiLCJpYXQiOjE0Mzg2NzI3MTMsImV4cCI6MTQzODY5MDcxM30.-2I_w8pJMeRjgOU-_07bX8HZVXyOuFIBbi8XBtvXTMU"');

  // load the service's module
  beforeEach(module('vivaverboApp'));

  // instantiate service
  let reviewService, repaso, tarjetasRepaso;
  let $httpBackend;
  ...
  beforeEach(inject(function (_reviewService_, $rootScope) {
    reviewService = _reviewService_;
    repaso = reviewService.repaso;
    tarjetasRepaso = reviewService.tarjetasRepaso;
  ...
  it('debe llegar al final', function() {
    ...
    for (let i = 0; i < total; i++) {
      ...
      if (0 === i % 2) {
        ...
        reviewService.borrar();
      }
      if (i < total - 1) {
        expect(repaso.finalizado).toBe(false, 'porque no hemos llegado a la última tarjeta');
        expect(repaso.tarjetaActual).toBe(i + 1);
      }
    }

    expect(repaso.finalizado).toBe(true);
    expect(repaso.tarjetaActual).toBe(total);
  });
});
